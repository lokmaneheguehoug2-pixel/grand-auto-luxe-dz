...
import { Calendar, Phone, User, Mail, Clock, MessageSquare, Loader as Loader2, CircleCheck as CheckCircle } from "lucide-react";
...
export function AppointmentBooking({ vehicleId, sellerId, sellerPhone, vehicleName }: AppointmentBookingProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const apptRef = push(ref(realtimeDb, "appointments"));
      ...
      await set(apptRef, {
        ...
        status: "pending",
        notification_email: adminEmail || null,
        created_at: new Date().toISOString(),
      });

      if (adminEmail) {
        const notifRef = push(ref(realtimeDb, "email_notifications"));
        await set(notifRef, {
          id: notifRef.key,
          to: adminEmail,
          subject: `New Viewing Appointment · ${vehicleName}`,
          body: `Client: ${form.client_name}\nPhone: ${form.client_phone}\nEmail: ${form.client_email || "N/A"}\nDate: ${form.preferred_date}\nTime: ${form.preferred_time}\nMessage: ${form.message || "N/A"}`,
          created_at: new Date().toISOString(),
          sent: false,
        });
      }
