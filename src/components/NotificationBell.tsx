...
export function NotificationBell() {
  ...
  const load = async () => {
    ...
    const [{ data: n }, { data: b }] = await Promise.all([
      supabase.from("notifications").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(15),
      supabase.from("broadcast_messages").select("id,title,body,created_at").order("created_at", { ascending: false }).limit(5),
    ]);
    const merged: Notif[] = [
      ...((n ?? []) as Notif[]),
      ...((b ?? []) as any[]).map((m) => ({ id: `b-${m.id}`, title: `📢 ${m.title}`, body: m.body, link: null, kind: "broadcast", read_at: null, created_at: m.created_at })),
